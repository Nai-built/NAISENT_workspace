# NAISENT Small Language Model in CUDA
...
modelId = CNL.Build(CNL.CHAIN([
    ...
    CNL.CHAIN([
        CNL.RMSNORM(d_model),
        CNL.MH_MASKED_SELF_ATTENTION(d_model, d_attention, d_heads),
        CNL.DENSE(d_model,d_model),
    ], residual=True),
    CNL.CHAIN([
        CNL.RMSNORM(d_model),
        CNL.DENSE(d_model,d_ffn, activation=CNL.AF_ReLU(0.001)),
        CNL.DENSE(d_ffn,d_model),
    ], residual=True),
    # STACK 2
    CNL.CHAIN([
        CNL.RMSNORM(d_model),
...
startTime = datetime.now()
load_model("NAISENT/NAISENT_SLM")
