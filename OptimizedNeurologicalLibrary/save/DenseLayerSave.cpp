...
DenseLayer::DenseLayer(int _inputSize, int _outputSize) {
...
void DenseLayer::Initialize(DenseLayer* denseLayer) {
	...
	neurologicalValue weights_initialization_boundary = (neurologicalValue)sqrt(6 / ((double)(denseLayer->inputSize) + (double)(denseLayer->outputSize)));
...
void dense_activation_relu(neurologicalValue* __restrict output, const neurologicalValue* __restrict biases
    , const int& batchSize, const int& outputSize
    , neurologicalValue* __restrict cached_output, neurologicalValue leakage)
{
    for (int b = 0; b < batchSize; ++b)
    {
...
void dense_activation_gradient_linear(const neurologicalValue* __restrict propagation, neurologicalValue* __restrict biases
    , const int& batchSize, const int& outputSize
...
void dense_activation_gradient_relu(neurologicalValue* __restrict propagation, neurologicalValue* __restrict biases
    , const int& batchSize, const int& outputSize
    , const neurologicalValue& rate
    , const neurologicalValue* __restrict cached_output, neurologicalValue leakage)
{
    for (int b = 0; b < batchSize; ++b)
    {
...
void dense_propagation_weights_kernel(const neurologicalValue* __restrict propagation, const neurologicalValue* __restrict cached_input, neurologicalValue* __restrict weights
    , const int& batchSize, const int& inputSize, const int& outputSize
    , const neurologicalValue& rate)
{
    for (int batchStart = 0; batchStart < batchSize; batchStart+=batch_BLOCK) {
    ...
    for (int inputStart = 0; inputStart < inputSize; inputStart+=input_BLOCK) {
        ...
        for (int b = batchStart; b < bmax; ++b)
        {
            ...
            for (int i = inputStart; i < imax; ++i)
            {
                ...
                neurologicalValue* __restrict w = weights + (i) * outputSize + outputStart;

                for (int o = 0; o < omax - outputStart; ++o) {
                    w[o] -= (p[o]*_cachedX) * rate;
                }
            }
        }
    }
...
void update_weights(neurologicalValue* weights, const neurologicalValue* weightsGradient
...
void update_biases(neurologicalValue* biases, const neurologicalValue* biasesGradient
    , const int& outputSize
    , const neurologicalValue& rate)
{
    for (int o = 0; o < outputSize; ++o)
        biases[o] -= biasesGradient[o] * rate;
}

void DenseLayer::ForwardPass(DenseLayer* denseLayer, const neurologicalConstantSpan input, const neurologicalSpan output) {
    int batchSize = input.size()/denseLayer->inputSize;
...
void DenseLayer::BackPropagation(DenseLayer* denseLayer, const neurologicalSpan propagation, const neurologicalSpan inputPropagation, const neurologicalValue& learnRate) {
    int batchSize = propagation.size()/denseLayer->outputSize;
